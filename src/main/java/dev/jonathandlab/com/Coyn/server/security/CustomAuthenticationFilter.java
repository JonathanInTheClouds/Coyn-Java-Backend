...
public class CustomAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException {
        ...
        AppUser appUser = appUserRepository.findAppUserByEmail(principal.getUsername())
                .orElseThrow(() -> new UsernameNotFoundException("User " + principal.getUsername() + " not found"));
        List<SimpleGrantedAuthority> authorities = appUser.getRoles().stream().map(AppUserRole::getName)
                .map(SimpleGrantedAuthority::new).toList();
        ServerTokenResponse serverTokenResponse = tokenService.createServerTokenResponse(appUser.getEmail(), authorities);
